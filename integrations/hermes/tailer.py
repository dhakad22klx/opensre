...
* The tailer does not parse content. It yields raw line strings (with
  trailing newlines stripped). Parsing happens in :mod:`integrations.hermes.parser`.

The tailer is single-threaded and pull-based (``__iter__`` blocks on
``poll_interval_s``); :class:`integrations.hermes.agent.HermesAgent` runs it on its
